...
function registerCommands() {
    ...
    let files = fs.readdirSync("./commands").filter((file) => file.endsWith(".js") && file != "example.command.js");
...
function registerEvents() {
    ...
    let files = fs.readdirSync("./events").filter((file) => file.endsWith(".js") && file != "example.event.js");

    for (const file of files) {
        const event = require(`./events/${file}`);
        events.push(event);

        if (event.once) discord.once(event.name, (...args) => event.execute(...args));
        else discord.on(event.name, (...args) => event.execute(...args));
...
function registerTasks() {
    ...
    let files = fs.readdirSync("./tasks").filter((file) => file.endsWith(".js") && file != "example.task.js");
